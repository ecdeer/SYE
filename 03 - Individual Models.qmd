...
# Ask - why bind rows instead of left join

Read Data and Left Join: 
```{r}
library(readxl)
...
clams_total <- clams |> left_join(covariate_data, by = "location")
clams_total
```
...
Plot: 
# here they are not on the same scale, so it makes it hard to compare

```{r}
covariate_data |> filter(body == "L")

library(patchwork)

(blacklake + horseshoe + lakeclear)  /
(lakeflower + longlake) /
(mirror + raquette + tupperhorseshoe) /
(tupperlakenorth + unionfallspond)
```

```{r}
covariate_data |> filter(body == "R")
```
