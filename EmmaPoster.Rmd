...
$$
L(t) = L \cdot (1 - e^{-(k + \beta \cdot x) \cdot (t-t_0)})
$$ where
...
-   $\beta$ adjusts the growth rate based on acidity

-   $x$ is binary that indicates acidity
...
# max_l plots
# river_maxl_line = ggplot(data = plot_df, aes(x = iter, y = max_l)) +
#  geom_line() 
...
# k plots
# river_k_line = ggplot(data = plot_df, aes(x = iter, y = k)) +
#  geom_line() 
...
# b plots
#river_b_line = ggplot(data = plot_df, aes(x = iter, y = b)) +
#  geom_line()

#river_b_hist = ggplot(data = plot_df, aes(x = b)) +
#  geom_histogram(colour = "skyblue4", fill = "skyblue1", bins = 15)

# sigma2 plots
#river_sigma2_line = ggplot(data = plot_df, aes(x = iter, y = sigma2_store)) +
#  geom_line()
...
# beta plots
#river_beta_line = ggplot(data = plot_df, aes(x = iter, y = beta_store)) +
#  geom_line()

#river_beta_hist = ggplot(data = plot_df, aes(x = beta_store)) +
#  geom_histogram(colour = "skyblue4", fill = "skyblue1", bins = 15)
```
...
compute_likelihood_log <- function(max_l, k, b, sigma2, beta) {
  dnorm(lakes$Length,
        mean =  vb_function(lakes$growth_period, max_l, k, b, beta, lakes$x),
        sd = sqrt(sigma2), log = TRUE) |>
    sum()
}
...
# max_l plots
#lakes_maxl_line = ggplot(data = plot_df, aes(x = iter, y = max_l)) +
#  geom_line() 
...
# k plots
# lakes_k_line = ggplot(data = plot_df, aes(x = iter, y = k)) + geom_line()
...
# b plots
#lakes_b_line = ggplot(data = plot_df, aes(x = iter, y = b)) +
#  geom_line()

#lakes_b_hist = ggplot(data = plot_df, aes(x = b)) +
#  geom_histogram(colour = "skyblue4", fill = "skyblue1", bins = 15)

# sigma2 plots
#lakes_sigma2_line = ggplot(data = plot_df, aes(x = iter, y = sigma2_store)) +
#  geom_line()
...
# beta plots
#lakes_beta_line = ggplot(data = plot_df, aes(x = iter, y = beta_store)) +
#  geom_line() 

#lakes_beta_hist = ggplot(data = plot_df, aes(x = beta_store)) +
#  geom_histogram(colour = "skyblue4", fill = "skyblue1", bins = 15)
```
...
results_river <- plot_df_river |>
  summarise(mean_l = mean(max_l),
            ...
            sd = sd(sigma2_store),
            mean_beta = mean(beta_store),
            sd_beta = sd(beta_store)) |>
  mutate(body = "River")
...
```{r}
# estimate sigma2 for rivers and lakes
river_sigma2_hist
lakes_sigma2_hist
```
...
- River clams have higher growth rates, max lengths, and sigma-squares than lake clams.

- This differences in parameters mean that river clams grow at a faster rate and achieve a longer length overall than lake clams.

- River water is less acidic than lake water. Clam shells are made out of calcium carbonate, which dissolve in acidic water. Differences in acidity between lake and rivers may partially explain the differences in shell length. 
