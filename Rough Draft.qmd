...
length_stats_river <- clams_predictions_waterbody |> 
  ...
  summarise(
    ...
    sd = sd(Length),
    max_growth_period = max(growth_period),
    body = body) |>
  slice(1)
...
```
Lakes have smaller average total length then rivers. However, they are both similarly distributed. 
...
```
Lake and River clams annual growths appear to slow at similar rates. 
...
ggplot(data = clams_predictions_waterbody,  aes(x = growth_period, y = Length)) +
  ...
  geom_smooth()
```
The growth curves for lakes vs. clams vary greatly. The have very different asymptotic lengths. This is because lake clams are much shorter than river clams. Additionally, it appears that lake clams start to reach their asymptotic length quicker than river clams.  
...
For this study, I built an algorithm designed to estimate the parameters of the von Bertanfly growth model while using a Bayesian framework to account for uncertainty in the data. In this algorithm, we want to estimate four parameters of the Von Bertanfly Growth model: max_l, k, b, and sigma-squared. The parameter max_l represents the maximum possible length for clams, k represents the growth rate, b represents time where length is 0, and sigma-squared represents the variance. The algorithm runs twice, once for river clams and once for lake clams in order to compare the estimate parameters and results between the two bodies of water. 

To begin, we have some data preparation where we clean the data, including data entry errors. 

Then, we create and define the von Bertanfly growth function to show shell length as a function of time using asymptotic length (max_l), growth rate (k), and time when length is zero (b). 
...
The parameters typically converge for both river and lake clams.
...
The parameters tested in the algorithm are relatively normal for river and lake clams. 
...
River clams are longer overall in comparison to lake clams. Lake clams stop growing sooner than river clams. 
